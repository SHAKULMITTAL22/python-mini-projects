...
class Test_TicTacToeCheckWin:
    def test_check_win_all_squares_same_player(self):
        squares = ['X', 'X', 'X', ' ', ' ', ' ', ' ', ' ', ' ']
        player = 'X'
        result = check_win(squares, player)
        assert result == True, "Should return True when all squares in a win condition are occupied by the same player."

    def test_check_win_no_condition_met(self):
        squares = ['X', 'O', 'X', 'X', 'O', ' ', ' ', ' ', ' ']
        player = 'X'
        result = check_win(squares, player)
        assert result == False, "Should return False when no winning condition is fully occupied by the same player."

    def test_check_win_partial_occupation(self):
        squares = ['X', 'X', ' ', ' ', ' ', ' ', ' ', ' ', ' ']
        player = 'X'
        result = check_win(squares, player)
        assert result == False, "Should return False when only some squares in a win condition are occupied by the same player."

    def test_check_win_mixed_players(self):
        squares = ['X', 'O', 'X', 'O', 'X', ' ', ' ', ' ', ' ']
        player = 'X'
        result = check_win(squares, player)
        assert result == False, "Should return False when squares in a win condition are occupied by different players."

    def test_check_win_exact_match(self):
        squares = ['X', 'X', 'X', 'O', 'O', ' ', ' ', ' ', ' ']
        player = 'X'
        result = check_win(squares, player)
        assert result == True, "Should return True when exactly the squares in one of the win conditions are occupied by the same player."
